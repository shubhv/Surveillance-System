...
int main (int argc, char * const argv[])
{
    ...
    myfile.open ( argv[2] ); //file storing the predicted tags
    result_file.open(argv[3]); //file storing the accuracy at different thresholds
    ground_truth.open(argv[4]); //file storing the ground truth tags
    string line; 
    int total_frames = 0;
    int accuracy = 0;
    int i = 0;
    ...
    int frame_number;
    const string DIR = "/home/ankita/ACADS/6th_sem/UGP/result/"; // directory where the images will be stored
    const string EXT = ".jpg"; // extension of the images
    const int DELAY = 500; // in mseconds, take a picture every 1/2 second
    const string LOGFILE = "/home/ankita/ACADS/6th_sem/UGP/result/log/";
    ...
    {       myfile << endl << endl <<"***************    "<<"THRESHOLD:   "<<th<<"  ******************"<<endl;
            accuracy = 0;
    // Set up camera
            CvCapture * camera = cvCreateFileCapture( argv[1] );
            ...
            while (frame_number<total_frames){
                ...
                threshold(motion, motion, th, 255, CV_THRESH_BINARY);
                ...
                if(number_of_changes>=there_is_motion)
                {
                    if(number_of_sequence>0){
                        myfile<< "f" <<"\t"<<frame_number<<endl;
                        if(tag[frame_number] == 'f')
                            accuracy++;
                        frame_number++;
                        cv::putText(current_frame, "detected: FOREGROUND", cv::Point(100, 100), fontFace, fontScale, Scalar::all(0), thickness,8);
                ...
                else
                {
                    ...
                    myfile<<"b"<<"\t"<<frame_number<<endl;
                    if(tag[frame_number] == 'b')
                        accuracy++;
                    frame_number++;
                    cv::putText(current_frame, "detected: BACKGROUND", cv::Point(100, 100), fontFace, fontScale, Scalar::all(255), thickness,8);
            ...
            float A = (float)accuracy/total_frames;
            result_file << "threshold = "<<th << "   accuracy = "<<A<<endl;
            if(A > maxA)
            {
                maxA = A;
                opt_th = th;
